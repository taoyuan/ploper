module.exports = function (plop) {
	...
	plop.setGenerator('test', {
		...
		actions: [
			{
				type: 'add',
				path: plop.resolve('folder/{{dashCase name}}.txt'),
			...
			function customAction(answers) {
				...
				if (fs.existsSync(plop.resolve(changeFile))) {
					// returned value shows up in the console
					return existsMsg;
				} else {
					// do a synchronous copy via node fs
					fs.writeFileSync(plop.resolve(changeFile), fs.readFileSync('templates/' + changeFile));
					return copiedMsg;
				}
			},{
				type: 'modify',
				path: plop.resolve('change-me.txt'),
	...
	plop.setGenerator('custom-prompt', {
		...
		actions: [
			{
				type: 'add',
				path: plop.resolve('{{path}}/{{fileName}}.txt'),
	...
	plop.setGenerator('dynamic actions', {
		...
		actions: function(data) {
			var actions = [
				{
					type: 'add',
					path: plop.resolve('folder/{{dashCase name}}-burger.txt'),
			...
			if(data.hasPotatoes) {
				actions = actions.concat([
					{
						type: 'add',
						path: plop.resolve('folder/{{dashCase name}}-potatoes.txt'),
